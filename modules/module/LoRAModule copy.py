...
class DoRAModule(LoRAModule):
    ...
    def initialize_weights(self):
        ...
        orig_weight_flat = orig_weight.reshape(orig_weight.shape[0], -1)
        norms = torch.norm(orig_weight_flat, dim=1, keepdim=True)
        self.dora_scale = nn.Parameter(
            ...
            norms.reshape(orig_weight.shape[0], *[1] * self.dora_num_dims)
            .to(device=self.orig_module.weight.device),
            requires_grad=True
        )
    ...
    def forward(self, x, *args, **kwargs):
        ...
        #          .transpose(0, 1) + eps
        wp_flat = WP.detach().reshape(WP.shape[0], -1)
        norm = wp_flat.norm(dim=1, keepdim=True)
        norm = norm.reshape(WP.shape[0], *[1] * self.dora_num_dims) + eps
